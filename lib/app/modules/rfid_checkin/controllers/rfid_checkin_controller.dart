...
class RfidCheckinController extends GetxController
  ...
  Future<void> _initializeImageCache() async {
    try {
      ...
      AppLogger.info('RfidCheckinController',
          'Servicio de caché de imágenes inicializado');
    } catch (e) {
      AppLogger.error(
          'RfidCheckinController', 'Error inicializando caché de imágenes', e);
    }
  }
  ...
  Future<void> _precacheActiveUserImages() async {
    try {
      AppLogger.info('RfidCheckinController',
          'Iniciando precarga de imágenes de usuarios activos (RFID)');
      ...
      if (activeUsers.isEmpty) {
        AppLogger.warning(
            'RfidCheckinController', 'No hay usuarios para precargar');
        return;
      }
      ...
      AppLogger.info(
          'RfidCheckinController', 'Precargando imágenes de usuarios activos');
      ...
      for (final user in usersToCache) {
        try {
          ...
          AppLogger.warning(
              'RfidCheckinController', 'Error precargando imagen');
        }
      }
      ...
      AppLogger.error(
          'RfidCheckinController', 'Error en precarga de imágenes', e);
    }
  }
  ...
  Future<void> checkAccessByRfid(String rfidCode) async {
    ...
    try {
      ...
      final UserModel? user =
          allUsers.firstWhereOrNull((user) => user.rfidCard == rfidCode);
      ...
      if (user == null) {
        ...
        _sendMembershipStatusToESP32(
            rfidCode, membershipStatus, user.name, 'denied', 'rfid');
        ...
        _sendMembershipStatusToESP32(
            rfidCode, membershipStatus, user.name, accessType, 'rfid');
      } else {
        // Membresía activa
      ...
      if (membershipStatus == RfidConfig.membershipActive ||
          membershipStatus == RfidConfig.membershipExpiring) {
      ...
      AppLogger.error(
          'RfidCheckinController', 'Error al procesar acceso con RFID', e);
    ...
    Future(() async {
      try {
        ...
        if (user.id != null) {
          ...
          AppLogger.info('RfidCheckinController',
              'Registro de acceso guardado en segundo plano');
        }
      } catch (e) {
        AppLogger.error('RfidCheckinController',
            'Error al registrar acceso en segundo plano', e);
      }
    });
  }
  ...
  void _sendMembershipStatusToESP32(
    ...
    Future(() async {
      try {
        ...
        AppLogger.error(
            'RfidCheckinController', 'Error enviando estado al ESP32', e);
      }
    });
  }
  ...
  void updateReaderIpAddress(String newIp) {
    ...
    RfidConfig.forceUpdateIP(
        newIp); // Usar la IP sin formato para el método interno

    AppLogger.info(
        'RfidCheckinController', 'Dirección IP del lector RFID actualizada');
  ...
  Future<String?> _registerAccessInSupabase(
      UserModel user, String method) async {
    try {
      if (user.id == null) {
        AppLogger.error('RfidCheckinController',
            'No se puede registrar acceso: ID de usuario nulo');
        return null;
      }
      ...
      AppLogger.error('RfidCheckinController',
          'Excepción al registrar acceso en Supabase', e);
      return null;
    }
  }
}
